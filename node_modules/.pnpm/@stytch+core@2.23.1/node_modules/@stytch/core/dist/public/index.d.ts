...
interface IHeadlessSessionClient {
    ...
    /**
     * Returns information about the current session, including the session object
     * (if available) and whether the data was retrieved from the cache.
     */
    getInfo(): SessionInfo;
    /**
...
interface IHeadlessUserClient {
    ...
    getSync(): User | null;
    /**
     * Returns information about the current user, including the user object (if
     * available) and whether the data was retrieved from the cache.
     */
    getInfo(): UserInfo;
    /**
...
type B2BAllowedAuthMethods = "sso" | "magic_link" | "password" | "google_oauth" | "microsoft_oauth";
...
interface IHeadlessB2BSessionClient {
    ...
    /**
     * Wraps Stytch's {@link https://stytch.com/docs/b2b/api/revoke-session revoke} Session endpoint and revokes all Sessions of another Member.
     *
     ...
     *
     * @example
     * stytch.sessions.revokeForMember({ member_id: 'member-id-123' });
    ...
    /**
     *  Updates the current session with a sessionToken. After setting this, you will need to make an {@link https://stytch.com/docs/b2b/api/authenticate-session authenticate } call to authenticate the session and retrieve the latest state.
     *
     * @param tokens - The session tokens to update to
     */
    updateSession(tokens: SessionTokensUpdate): void;
...
interface IHeadlessB2BDiscoveryClient {
    organizations: {
         ...
         *
         * If there is a current Member Session, the SDK will call the endpoint with the session token. Otherwise, the SDK will use the intermediate session token.
...
interface IHeadlessB2BSelfClient {
    ...
    getSync(): Member | null;
    /**
     * Returns information about the current member, including the member object
     * (if available) and whether the data was retrieved from the cache.
     */
    getInfo(): MemberInfo;
    /**
    ...
    /**
     * The delete totp method wraps the {@link https://stytch.com/docs/b2b/api/delete-member-mfa-totp delete totp} API endpoint.
     * The caller must have permission to call this endpoint via the project's RBAC policy & their role assignments.
     *
    ...
    /**
     * The unlink retired email method wraps the {@link https://stytch.com/docs/b2b/api/unlink-retired-member-email unlink retired email} API endpoint.
     *
...
interface IHeadlessB2BMemberClient {
     ...
     * The delete MFA phone number method wraps the {@link https://stytch.com/docs/b2b/api/delete-member-mfa-phone-number delete phone number} API endpoint.
     *
     * See the {@link https://stytch.com/docs/b2b/sdks/javascript-sdk/members#delete-member-mfa-phone-number-deprecated Stytch Docs} for a complete reference.
     *
...
interface IHeadlessB2BOrganizationClient {
    ...
    /**
     * The synchronous method for getting an organization. This is the recommended approach. You can listen to changes with the {@link onChange onChange} method.
     * If logged in, this returns the cached organization object, otherwise it returns null. This method does not refresh the organization's data.
     * The `@stytch/react` library provides the `useStytchOrganization` hook that implements these methods for you to easily access the organization and listen for changes.
     *
    ...
    /**
     * Returns information about the current organization, including the
     * organization object (if available) and whether the data was retrieved from
     * the cache.
     */
    getInfo(): OrganizationInfo;
    /**
     * Allows listening to changes in the organization object when using {@link getOrganizationSync getOrganizationSync()}.
     *
    ...
    members: {
        ...
        /**
         * Deletes a Member's TOTP MFA method.
         * The delete totp method wraps the {@link https://stytch.com/docs/b2b/api/delete-member-mfa-totp delete totp} API endpoint.
         * The caller must have permission to call this endpoint via the project's RBAC policy & their role assignments.
         *
        ...
        /**
         * Deletes an email that has previously been retired.
         * The email to be deleted must be identified by its id or its address.
         * The unlink retired email method wraps the {@link https://stytch.com/docs/b2b/api/unlink-retired-member-email unlink API endpoint}.
         * The caller must have permission to call this endpoint via the project's RBAC policy & their role assignments.
         *
         * See the {@link https://stytch.com/docs/b2b/sdks/javascript-sdk/members#unlink-retired-member-email Stytch Docs} for a complete reference.
         *
...
interface IHeadlessB2BTOTPsClient {
    /**
     * The authenticate method wraps the {@link https://stytch.com/docs/b2b/api/authenticate-totp TOTP authenticate} API endpoint.
     *
    ...
    /**
     * Wraps Stytch's {@link https://stytch.com/docs/b2b/api/totp-create create} endpoint. Call this method to create a new TOTP instance for a member. The member can use the authenticator application of their choice to scan the QR code or enter the secret.
     * You can listen for successful member updates anywhere in the codebase with the `stytch.member.onChange()` method or `useStytchMember()` hook if you are using React.
     *
     * @example
     * ```
...
interface IHeadlessB2BRecoveryCodesClient {
    /**
     * Wraps Stytch's {@link https://stytch.com/docs/b2b/api/recovery-codes-recover recover} endpoint. Authenticates a member using a recovery code.
     *
     * @example
     * ```
    ...
    /**
     * Wraps Stytch's {@link https://stytch.com/docs/b2b/api/recovery-codes-rotate rotate} endpoint.  Rotates a member's recovery codes. This invalidates all existing recovery codes and generates a new set of recovery codes.
     *
     * @example
     * ```
    ...
    /**
     * Gets a member's recovery codes. {@link https://stytch.com/docs/b2b/api/recovery-codes-get Get} endpoint.
     *
     * @example
     * ```
...
interface IHeadlessB2BSCIMClient {
     ...
     * This method wraps the {@link https://stytch.com/docs/b2b/api/get-scim-connection get-connection} endpoint.
     ...
     * This method wraps the {@link https://stytch.com/docs/b2b/api/get-scim-connection-groups get-connection-groups} endpoint.
     * The caller must have permission to view SCIM via the project's RBAC policy & their role assignments.
     *
     * @example
     ...
     * This method wraps the {@link https://stytch.com/docs/b2b/api/scim-rotate-token-complete complete-rotation} endpoint.
     * The caller must have permission to modify SCIM via the project's RBAC policy & their role assignments.
     *
     * @example
...
interface IHeadlessBiometricsClient {
    ...
    /**
     * Clears existing biometric registrations stored on the device.
     */
