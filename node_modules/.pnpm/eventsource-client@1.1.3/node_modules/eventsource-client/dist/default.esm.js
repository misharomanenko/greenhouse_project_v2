...
function createEventSource$1(optionsOrUrl, { getStream: getStream2 }) {
  const options = typeof optionsOrUrl == "string" || optionsOrUrl instanceof URL ? { url: optionsOrUrl } : optionsOrUrl, { onMessage, onConnect = noop, onDisconnect = noop, onScheduleReconnect = noop } = options, { fetch, url, initialLastEventId } = validate(options), requestHeaders = { ...options.headers }, onCloseSubscribers = [], subscribers = onMessage ? [onMessage] : [], emit = (event) => subscribers.forEach((fn) => fn(event)), parser = createParser({ onEvent, onRetry });
  ...
  function connect() {
    request || (readyState = CONNECTING, controller = new AbortController(), request = fetch(url, getRequestOptions()).then(onFetchResponse).catch((err) => {
      request = null, !(err.name === "AbortError" || err.type === "aborted") && scheduleReconnect();
    }));
  }
  function close() {
